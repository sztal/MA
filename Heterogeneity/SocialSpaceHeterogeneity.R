...
resmob.disc = lm(resmob ~ (ent_avg+I(ent_avg^2))*attdiscovered, data=D)
# significatn interaction term
...
xyplot(resmob ~ ent_avg, data=D,
       panel = function(x, y) {
             panel.xyplot(x, y)
             panel.loess(x, y, col="red")
             panel.curve(6.1799+7.3954*x-3.2294*x^2)
