...
Jak widać dwa pierwsze wymiary zachowują aż 67,7% wariancji, co jest zdecydowanie dobrym wynikiem. Co więcej praktycznie wszystkie zmienne i kategorie odpowiedzi (poza dosłownia kilkoma przypadkami) charakteryzują się wysoką lub bardzo wysoką jakością reprodukcji na dwuwymiarowej płaszczyźnie rozwiązania (wartość qlt). Co więcej dosyć równomiernie rozłożone wartości udziału punktów w wymiarach (wartość ctr) pokazują, że rozwiązanie nie jest patologicznie zdominowane przez tylko jedną bądź kilka kategorii odpowiedzi.
...
par(mar=c(4,4,1,1))
makeMCAplot(D, cc.mca, cex=.8, zoomout=1.5, xlab="Wymiar I", ylab="Wymiar II")
par(mar=c(4,4,4,4))
```
...
```{r}
cc.ind.coord = projectInd(D, cc.mca)
D.back$cultcap = cc.ind.coord[,1] * (-1) # wyniki są odwrócone, żeby nowo zapisania zmienna odpowidała kapitałowi kulturowemu a nie jego przeciwieństwu
```
